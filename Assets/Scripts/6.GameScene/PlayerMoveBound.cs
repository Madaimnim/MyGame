...
public class PlayerMoveBound : MonoBehaviour
{
    ...
    private IEnumerator Start() {
        ...
        {
            Debug.Log($"📦 PlayerStateManager 中目前 activePlayersDtny 數量：{PlayerStateManager.Instance.activePlayersDtny.Count}");
            foreach (var playerObj in PlayerStateManager.Instance.activePlayersDtny.Values)
            {
                if (playerObj == null)
                {
                    Debug.LogWarning("⚠ 有 playerObj 為 null！");
                    continue;
                }
                ...
                Debug.Log($"✅ 設定 {playerObj.name} 的邊界成功");
            }
